...
\begin{marginfigure}
      \includegraphics{../Ch-orgo/figure1}
   \end{marginfigure}
...
\rowcolor{black!45}
\toprule
\multicolumn{5}{l}{\hypersetup{colorlinks,linkcolor={white}} \cellcolor{black}\color{white}\bfseries\small Table \ref{tab:{\chapterlabel}1} Prefixed for alkane naming } \\
\midrule
...
\end{tabular}\end{center}
   ...
   \begin{tikzpicture} \node (a) at (0,0) {\includegraphics[width=4cm, height=4cm]{../Ch-orgo/figure2}} node[rotate=90, font=\tiny] at ([yshift=.5cm,xshift=.1cm]a.south east) {\textsuperscript{\textcopyright} www.wallpaperflare.com} ;
\node[text width=5cm] at ([yshift=0.4cm]a.north) {\mytriangle{red}Methane (\ce{CH4}) is used as a fuel for ovens, homes, water heaters.};
\end{tikzpicture}
\end{marginfigure}
   \begin{marginfigure}[2cm]
\begin{tikzpicture} \node (a) at (0,0) {\includegraphics[width=4cm, height=4cm]{../Ch-orgo/figure3}} node[rotate=90, font=\tiny] at ([yshift=.5cm,xshift=.1cm]a.south east) {\textsuperscript{\textcopyright} www.wallpaperflare.com} ;
\node[text width=5cm] at ([yshift=0.4cm]a.north) {\mytriangle{red}Hydrocarbons are made of carbon and hydrogen.};
\end{tikzpicture}
\end{marginfigure}
   \begin{marginfigure}[2cm]
\begin{tikzpicture} \node (a) at (0,0) {\includegraphics[width=4cm, height=4cm]{../Ch-orgo/figure4}} node[rotate=90, font=\tiny] at ([yshift=.5cm,xshift=.1cm]a.south east) {\textsuperscript{\textcopyright} wikipedia} ;
\node[text width=5cm] at ([yshift=1.0cm]a.north) {\mytriangle{red}The octane rating of gas is a standard measure of the performance of engine fuels, originally determined by mixing a gasoline made entirely of heptane and 2,2,4-trimethylpentane (a highly branched octane).};
\end{tikzpicture}
...
\item[\docfilehook{  General formula for alkanes}{General formula for alkanes}] At this point we saw two different alkanes: methane (\ce{CH4}), butane (\ce{C2H6}) and propane (\ce{C3H8}). The general formula for an alkane with $n$ carbon atoms is:
...
\begin{flushright}  \mytriangleright{dgreen}\small Answer: (decane) \ce{C10H22}; (pentane) \ce{C5H12} \end{flushright}
...
\begin{flushright} \mytriangleright{dgreen}\small Answer: heptane, \ce{C9H20}.\end{flushright}\end{example}%%%%%%%%%%%%%%%%%%%%%%%% EXAMPLE BOX
...
\begin{flushright}  \mytriangleright{dgreen} \small Answer:  \setpolymerdelim()\chemfig{CH_3 -[@{A,0.5}:0,2]CH_2 -[@{B,0.5}:0,2]CH_3 }\makebraces(5pt,5pt){$\scriptstyle\!\!3$}{A}{B} \end{flushright}
\faDiamond\ \textlcsc{ \textcolor{dgreen}{\Large \textbf{Study Check}} }\\
...
\sloppy
   ...
   \begin{marginfigure}[2cm]
   \begin{tikzpicture} \node (a) at (0,0) {\includegraphics[width=4cm, height=4cm]{../Ch-orgo/figure5}} node[rotate=90, font=\tiny] at ([yshift=.5cm,xshift=.1cm]a.south east) {\textsuperscript{\textcopyright} www.wallpaperflare.com} ;
\node[text width=5cm] at ([yshift=0.4cm]a.north) {\mytriangle{red}Cyclononane contains 9 carbons in a cycle. \\ \textcolor{red}{\textbf{Q:} What is the molecular formula Cyclononane?}};
\end{tikzpicture}
\end{marginfigure}
   \begin{marginfigure}[2cm]
\begin{tikzpicture} \node (a) at (0,0) {\includegraphics[width=4cm, height=4cm]{../Ch-orgo/figure6}} node[rotate=90, font=\tiny] at ([yshift=.5cm,xshift=.1cm]a.south east) {\textsuperscript{\textcopyright} www.wallpaperflare.com} ;
\node[text width=5cm] at ([yshift=0.4cm]a.north) {\mytriangle{red}Cyclohexane has a chair shape. \\ \textcolor{red}{\textbf{Q:} How many H are there in cyclohexane?}};
 \end{tikzpicture}
\end{marginfigure}
   \begin{marginfigure}[2cm]
\begin{tikzpicture}  \node (a) at (0,0) {\includegraphics[width=2.7cm, height=2.7cm]{../Ch-orgo/figure7}} node[rotate=90, font=\tiny] at ([yshift=.5cm,xshift=.1cm]a.south east) {\textsuperscript{\textcopyright} wikipedia} ;
\node[text width=5cm] at ([yshift=1.0cm]a.north) {\mytriangle{red}Cyclopropane has only three carbons and a lot tension in its cycle. \\ \textcolor{red}{\textbf{Q:} Would cyclopropane be a stable of unstable molecule?}};
\end{tikzpicture}
%\caption{Examples of measuring equipment}
...
\begin{description}
\item[\docfilehook{  Cyclic alkanes}{Cyclic alkanes}] Let us use the expanded estructure of hexane as an example. 
...
\item[\docfilehook{  Naming cyloalkanes}{Naming cyloalkanes}] The naming of alkanes and cycloalkanes is very similar. You just need to add the cyclo prefix to the name. For example, the alkane with five carbons is called pentane. The corresponding cycloalkane is called cyclopentane:
...
\begin{flushright} \mytriangleright{dgreen}\small Answer: \chemfig{CH_2*4(-CH_2-CH_2-CH_2-[,,2])} \end{flushright}
\faDiamond\ \textlcsc{ \textcolor{dgreen}{\Large \textbf{Study Check}} }\\
Write down the condensed structure and name the following cycloalkane:
\begin{center}\chemfig{*3(---)} \end{center}
\begin{flushright} \mytriangleright{dgreen}\small Answer: cyclopropane, \chemfig{CH_2*3(-CH_2-CH_2-[,,2])} \end{flushright}
\end{example}%%%%%%%%%%%%%%%%%%%%%%%% EXAMPLE BOX
...
\\
\textlcsc{ \textcolor{dgreen}{\Large \textbf{Solution}} }\\
Using Equation \ref{formula12.2} we have that the molecular formula for cyclodecane ($n=10$) would be: \ce{C10H20}. Similarly, the molecular formula for cyclopentane ($n=5$) would be: 
\begin{flushright} \mytriangleright{dgreen}\small Answer: \ce{C5H10}.\end{flushright}
\faDiamond\ \textlcsc{ \textcolor{dgreen}{\Large \textbf{Study Check}} }\\
Name the alkane with formula \ce{C7H14} and give the formula for cyclononane.
\begin{flushright} \mytriangleright{dgreen}\small Answer: cycloheptane, \ce{C9H18}.\end{flushright}
\end{example}%%%%%%%%%%%%%%%%%%%%%%%% EXAMPLE BOX
\end{description}
...
\item[\docfilehook{  Substituents}{Substituents}] There are many different substituents which can be found connected to an alkane chain. Their names are indicated in Figure 1.8. The easiest substituents are halogens and atoms of chorine (\chemfig{Cl-[:0,1]}), bromine (\chemfig{Br-[:0,1]}) or iodine (\chemfig{I-[:0,1]}) can connect to an alkane. The name of these substituents is chloro, bromo and iodo. Other substituents can contain carbon, like a methyl (\chemfig{CH_3-[:0,1]}) or a ethyl (\chemfig{CH_3CH_2-[:0,1]}). There are even more complex substituents such as tert-butyl




\item[\docfilehook{  Alkanes with a single substituent}{Alkanes with a single substituent}] Let us consider the following example. The condensed formula for propane is
...
\item[\docfilehook{  Alkanes with two or more equal substituents}{Alkanes with two or more equal substituents}] In the same way as when you have a single bromine atom attached to propane, you can also have two \chemfig{Br-[:0,1]}. In this case you need to use the prefix \emph{di} to indicate there are two identical bromines. For example, the name of the following molecule would be
...
\faDiamond\ \textlcsc{ \textcolor{dgreen}{\Large \textbf{Study Check}} }\\
Name the following hydrocarbon:
\begin{center} \chemfig{CH_3-C(-[:90,1]CH_3)(-[:-90,1]F)(-CH_2-I)}\end{center}\begin{flushright} \mytriangleright{dgreen}\small Answer: 1-iodo-2-fluoro-2-methylpropane. \end{flushright}
\end{example}%%%%%%%%%%%%%%%%%%%%%%%% EXAMPLE BOX
...
We have two methyl connected to carbon number three. Hence the final name will be: 
\begin{flushright} \mytriangleright{dgreen}\small Answer: 3-dimethylhexane. \end{flushright}
\faDiamond\ \textlcsc{ \textcolor{dgreen}{\Large \textbf{Study Check}} }\\
Name the following hydrocarbon:
\begin{center} \chemfig{CH_3-CH_2-C(-[:90]CH_2-CH_2-CH_2-CH_3)(-[:-90]CH_2-CH_2-CH_3)-CH_3} \end{center}
\begin{flushright} \mytriangleright{dgreen}\small Answer: 4-ethyl-4-methyloctane. \end{flushright}
\end{example}%%%%%%%%%%%%%%%%%%%%%%%% EXAMPLE BOX
...
\sloppy
\begin{description}
\item[\docfilehook{  Cylcoalkanes with one substituyent}{Cylcoalkanes with one substituyent}] Let us take a look at the following cycloalkane:
...
\item[\docfilehook{  Cylcoalkanes with two substituyent}{Cylcoalkanes with one substituyent}] Let us take a look at the following cycloalkane:
...
\item[\docfilehook{  Cylcoalkanes with repeated substituyent}{Cylcoalkanes with repeated substituyent}] Let us take a look at the following cycloalkane, which has two repeated fluorine substituents:
...
\sloppy
\begin{description}
\item[\docfilehook{  Alkene group: double bonds}{Alkene group: double bonds}] Alkenes contain at least one double bond between carbons. An example would be:
...
%\begin{marginfigure}[0cm]%%%%%%%% MARGIN FIGURE
%\includegraphics[width=0.4\columnwidth]{../Ch-orgo/figure8}
...
\item[\docfilehook{  Alkyne group: tripe bonds}{Alkyne group: tripe bonds}] Alkenes contain at least one tripple bond between carbons. An example would be:
...
\begin{center}\chemfig{R-C~C-R'}\hspace{0.5cm}\textcolor{blue}{Alkyne}\end{center}
 ...
 \begin{marginfigure}[-10cm]
   \begin{tikzpicture} \node (a) at (0,0) {\chemfig{?=[::+72]*5(-N=(-=[::-72]*5(-[,,,2]HN-[,,2](=-[::-36]*5(=N-(=-[::-72]*5(-NH-[,,1]?=-=))
-=-))-=-))-=-)}}  ;
\node[text width=5cm] at ([yshift=0.4cm]a.north) {\mytriangle{red}porphyrin has several amine groups. \\ \textcolor{red}{\textbf{Q:} How many amine groups has porphyrin?}};
\end{tikzpicture}
\end{marginfigure}
     \begin{marginfigure}[-2cm]
   \begin{tikzpicture} \node (a) at (0,0) {\chemfig{*6(-=-(-O-[::-60](-[::-60])=[::+60]O)=(-(=[::+60]O)-[::-60]OH)-=)}}  ;
\node[text width=5cm] at ([yshift=0.4cm]a.north) {\mytriangle{red}Aspirin contains an aromatic cycle, a carboxylic acid and a ester. \\ \textcolor{red}{\textbf{Q:} What is the molecular formula of aspirin?}};
\end{tikzpicture}
\end{marginfigure}
      \begin{marginfigure}[3cm]
   \begin{tikzpicture} \node (a) at (0,0) {\chemfig{*6((=O)-N(-CH_3)-*5(-N=-N(-CH_3)-=)--(=O)-N(-H_3C)-)}}  ;
\node[text width=5cm] at ([yshift=0.4cm]a.north) {\mytriangle{red}Caffeine contains amides and amines \\ \textcolor{red}{\textbf{Q:} How many different amines and amides are there in caffeine?}};
\end{tikzpicture}
\end{marginfigure}
       \begin{marginfigure}[4cm]
   \begin{tikzpicture} \node (a) at (0,0) {\chemfig{[:150]?*6(=*6(--*6(-N(-CH_3)--(-(=[::+60]O)-[::-60]N(-[::+60]-[::-60])-[::-60]-[::+60])-=)([::-120]<H)---)-*6(-=-=-(-[::-30,1.155]\chembelow{N}{H}?)=))}}  ;
\node[text width=5cm] at ([yshift=0.4cm]a.north) {\mytriangle{red}LSD has both an amide and an amide \\ \textcolor{red}{\textbf{Q:} How many different amines has LDS?}};
\end{tikzpicture}
%%\caption{Examples of measuring equipment}
%\label{fig:marginfig2}
\end{marginfigure}
...
\begin{center}\chemfig{**6(-(-CH_3)-----)}\hspace{1cm} or \hspace{1cm}\chemfig{**6(---(-H_2C-[-1]CH_3)---)} and \hspace{1cm} \chemfig{*6((-H)-(-H)=(-H)-(-H)=(-H)-(-F)=)}  \end{center}
   ...
   \begin{marginfigure}[-2cm]
   \begin{tikzpicture} \node (a) at (0,0) {\includegraphics[width=4cm, height=4cm]{../Ch-orgo/figure9}} node[rotate=90, font=\tiny] at ([yshift=.5cm,xshift=.1cm]a.south east) {\textsuperscript{\textcopyright} wikipedia} ;
\node[text width=5cm] at ([yshift=1.0cm]a.north) {\mytriangle{red}Esters give flavor to many fruits.\\ \textcolor{red}{\textbf{Q:} What is the formula for the smallest ester?}};
\end{tikzpicture}
\end{marginfigure}
     \begin{marginfigure}[2cm]
   \begin{tikzpicture}\node (a) at (0,0) {\includegraphics[width=4cm, height=4cm]{../Ch-orgo/figure10}} node[rotate=90, font=\tiny] at ([yshift=.5cm,xshift=.1cm]a.south east) {\textsuperscript{\textcopyright} wikipedia} ;
\node[text width=5cm] at ([yshift=1.0cm]a.north) {\mytriangle{red}Vinegar contains acetic acid, the smallest carboxylic acic.\\ \textcolor{red}{\textbf{Q:} What is the formula for acetic acid?}};
\end{tikzpicture}
\end{marginfigure}
     \begin{marginfigure}[2cm]
   \begin{tikzpicture} \node (a) at (0,0) {\includegraphics[width=4cm, height=4cm]{../Ch-orgo/figure14}} node[rotate=90, font=\tiny] at ([yshift=.5cm,xshift=.1cm]a.south east) {\textsuperscript{\textcopyright} wikipedia} ;
\node[text width=5cm] at ([yshift=1.0cm]a.north) {\mytriangle{red}Diethyl ether was formerly used as a general anesthetic, until non-flammable drugs were developed. };
%%\caption{Examples of measuring equipment}
%\label{fig:marginfig2}
\end{tikzpicture}
\end{marginfigure}
...
\item[\docfilehook{  Alcohol, ether and thiols group}{Alcohol, ether and thiols group}] Alcohols contain an \ce{-OH} group attached to a carbon. 
...
\item[\docfilehook{  Aldehydes and ketones}{Aldehydes and ketones}] Ketones and aldehydes both contain a \ce{C=O} group. Still these are two different groups and ketones have a \ce{C=O} group bounded to two different carbon atoms
...
\textlcsc{ \textcolor{dgreen}{\Large \textbf{Solution}} }\\
The left molecule is a ketone as the carbonyl group (\ce{C=O}) is connected to a \ce{CH3} and a  \ce{CH2}. Differently, the right molecule is an aldehyde as the carbonyl group is connected to a cycloalkane but also to a hydrogen. 
...
\item[\docfilehook{  Carboxylic acids and esters}{Carboxylic acids and esters}] Carboxylic acids contain a carbonyl group (\ce{C=O}) connected to an hydrocarbon and also an alcohol group:
...
\begin{center}\chemfig{R-C(=[:90]O)-O-R'}\hspace{0.5cm}\textcolor{blue}{Ester}\end{center}
   ...
   \begin{marginfigure}[0cm]
   \begin{tikzpicture} \node (a) at (0,0) {\includegraphics[width=4cm, height=4cm]{../Ch-orgo/figure11}} node[rotate=90, font=\tiny] at ([yshift=.5cm,xshift=.1cm]a.south east) {\textsuperscript{\textcopyright} www.wallpaperflare.com} ;
\node[text width=5cm] at ([yshift=0.4cm]a.north) {\mytriangle{red}Cinnamaldehyde is the aldehyde that gives cinnamon its flavor and odor.};
%\begin{center}\hspace{1cm}\chemfig{**6(---(-[:45]=[:-45]-[:45](=[:90]O)-[:-45]H)---)}\end{center}
\end{tikzpicture}
\end{marginfigure}
   \begin{marginfigure}[2cm]
\begin{tikzpicture} \node (a) at (0,0) {\includegraphics[width=4cm, height=4cm]{../Ch-orgo/figure15}} node[rotate=90, font=\tiny] at ([yshift=.5cm,xshift=.1cm]a.south east) {\textsuperscript{\textcopyright} www.wallpaperflare.com} ;
\node[text width=5cm] at ([yshift=0.4cm]a.north) {\mytriangle{red}Benzyl acetate is an ester that flavors pears.};
%\begin{center}\hspace{1cm}\chemfig{**6(---(-[:45]-[:-45]O-[:45](=[:90]O)-[:-45])---)}\end{center}
\end{tikzpicture}
\end{marginfigure}
   \begin{marginfigure}[2cm]
\begin{tikzpicture} \node (a) at (0,0) {\includegraphics[width=4cm, height=4cm]{../Ch-orgo/figure16}} node[rotate=90, font=\tiny] at ([yshift=.5cm,xshift=.1cm]a.south east) {\textsuperscript{\textcopyright} wikipedia} ;
\node[text width=5cm] at ([yshift=1.0cm]a.north) {\mytriangle{red}Butyl butyrate is an ester that flavors pineapple.};
%\begin{center}\hspace{1cm}\chemfig{-[:45]-[:-45]-[:45](=[:90]O)-[:-45]O-[:45]-[:-45]-[:45]-[:-45]}\end{center}

\end{tikzpicture}
\end{marginfigure}
   \begin{marginfigure}[1cm]
\begin{tikzpicture} \node (a) at (0,0) {\includegraphics[width=4cm, height=4cm]{../Ch-orgo/figure17}} node[rotate=90, font=\tiny] at ([yshift=.5cm,xshift=.1cm]a.south east) {\textsuperscript{\textcopyright} wikipedia} ;
\node[text width=5cm] at ([yshift=1.0cm]a.north) {\mytriangle{red}Ethyl isovalerate is an ester that flavors apples.};
...
\begin{flushright} \mytriangleright{dgreen}\small Answer: (left) carboxylic acid; (right) ester. \end{flushright}

\faDiamond\ \textlcsc{ \textcolor{dgreen}{\Large \textbf{Study Check}} }\\
Classify the following molecules as an carboxylic acid or ester:
\begin{center}  \chemfig{*6(-(-[:-90]C(=[:-45]O)(-[:-135]O-CH_3))-----)}  \hspace{0.5cm}  \chemfig{*6(---(-C(=[:-45]O)(-[:45]OH))---)}   \end{center}

\begin{flushright} \mytriangleright{dgreen}\small Answer: (left) ester; (right) acid. \end{flushright}\end{example}%%%%%%%%%%%%%%%%%%%%%%%% EXAMPLE BOX

%\begin{marginfigure}[0cm]%%%%%%%% MARGIN FIGURE
%\includegraphics[width=1\columnwidth]{../Ch-orgo/figure11}\vspace{0.5cm}
%%\includegraphics[width=1\columnwidth]{../Ch-orgo/figure12}
%\begin{center}\hspace{1cm}\chemfig{**6(---(-[:45]=[:-45]-[:45](=[:90]O)-[:-45]H)---)}\end{center}
%\caption{Cinnamaldehyde is the aldehyde that gives cinnamon its flavor and odor.}
%\end{marginfigure}%%%%%%% MARGIN FIGURE
%\begin{marginfigure}[0cm]%%%%%%%% MARGIN FIGURE
%\includegraphics[width=0.75\columnwidth]{../Ch-orgo/figure15}\vspace{0.5cm}
%\begin{center}\hspace{1cm}\chemfig{**6(---(-[:45]-[:-45]O-[:45](=[:90]O)-[:-45])---)}\end{center}
%\caption{Benzyl acetate is an ester that flavors pears.}
%\end{marginfigure}%%%%%%% MARGIN FIGURE
%\begin{marginfigure}[0cm]%%%%%%%% MARGIN FIGURE
%\includegraphics[width=0.75\columnwidth]{../Ch-orgo/figure16}\vspace{0.5cm}
%\begin{center}\hspace{1cm}\chemfig{-[:45]-[:-45]-[:45](=[:90]O)-[:-45]O-[:45]-[:-45]-[:45]-[:-45]}\end{center}
%\caption{Butyl butyrate is an ester that flavors pineapple.}
%\end{marginfigure}%%%%%%% MARGIN FIGURE
%%\begin{marginfigure}[0cm]%%%%%%%% MARGIN FIGURE
%%\includegraphics[width=0.75\columnwidth]{../Ch-orgo/figure17}\vspace{0.5cm}
%%\begin{center}\hspace{1cm}\chemfig{-[:45](-[:90])-[:-45]-[:45](=[:90]O)-[:-45]O-[:45]-[:-45]-[:45]-[:-45]}\end{center}
...
\item[\docfilehook{  Amines and amides}{Amines and amides}] Amines and amides are groups containing nitrogen. Amines are derivative of ammonia (\ce{NH3}) with one or more of the hydrogen atoms being replaced by a hydrocarbon
...
\faDiamond\ \textlcsc{ \textcolor{dgreen}{\Large \textbf{Study Check}} }\\
Classify the following molecules as an amide or amine:
\begin{center} \chemfig{*6(---(-N(-[:-45]CH_3)(-CH_3))---)} \hspace{0.5cm}   \chemfig{*6(-(-[:-90]C(=[:-45]O)(-[:-135]N(-CH_3)(-[:180]CH_3)))-----)}     \end{center}
