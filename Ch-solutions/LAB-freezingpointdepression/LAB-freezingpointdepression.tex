\documentclass[cover.tex]{subfiles}
\newcommand\chapterlabel{../Ch-solutions/LAB-freezingpointdepression}
\begin{document} 
\begin{refsection}


%%%%%%%%%%%%HEADING TITLE
\hfill\vspace{0.2cm}\begin{center}{\large \bfseries EXPERIMENT {\thesection}\par\Huge
 \import{\chapterlabel/}{title}
 \\[5pt] \par}\vspace{0.2cm}\end{center}\par\noindent
 %%%%%%%%%%%%HEADING
 
 
\section*{Goal}
 \import{\chapterlabel/}{goal}
\section*{Materials}
 \import{\chapterlabel/}{materials}
\section*{Background}
%\import{\chapterlabel/}{video}
\import{\chapterlabel/}{background}


%\import{\chapterlabel/}{examplebox}
%%%%%BIBLIOGRAPHY
\printbibliography[heading=subbibliography]
%%%%%BIBLIOGRAPHY
%\section*{Lab Information}
% \begin{itemize}[label={$\blacksquare$}]
 %  \import{\chapterlabel/}{labinfo}
% \end{itemize}
\section*{Procedure}
\import{\chapterlabel/}{procedure} 
\clearpage\mbox{}\clearpage

 




%%%%%%%%%%%%HEADING
\newgeometry{right=5cm}\thispagestyle{sidebar}\sidebar{   \vspace{1em}\vspace{1cm} \vfill \par\vspace*{1em}}\restoregeometry\clearpage \begin{multicols}{2}\begin{tcolorbox}[enhanced jigsaw,breakable,size=title,colback=mybrown!05,colframe=black,fonttitle=\bfseries,title=STUDENT INFO,pad at break=1mm, break at=15cm/0pt ]\vspace{0.2cm}\noindent Name:  \begin{Form}\TextField[name=NAME, width=4cm,height=0.5cm,multiline=false, borderstyle=U, bordercolor={black}]{} \end{Form}   \hspace{0.2cm}Date:  \begin{Form}\TextField[name=DATE, width=1.5cm,height=0.5cm,multiline=false, borderstyle=U, bordercolor={black}]{}\end{Form} \\\end{tcolorbox}\end{multicols}\hfill\vspace{0.2cm}\begin{center}{\large \bfseries Pre-lab Questions \par\Huge
\import{\chapterlabel/}{title}
\\[5pt] \par}\vspace{0.2cm}\end{center}\par\noindent\uline{  \hfill \normalsize \hfill       }
%%%%%%%%%%%%HEADING
\import{\chapterlabel/}{prelab}
% \clearpage\mbox{}\clearpage

%%%%%%%%%%%%HEADING
\newgeometry{right=5cm}\thispagestyle{sidebar}\sidebar{ \vspace{1em} \vspace{1cm}\vfill\par\vspace*{1em}}\restoregeometry\clearpage\begin{multicols}{2}\begin{tcolorbox}[enhanced jigsaw,breakable,size=title,colback=mybrown!05,colframe=black,fonttitle=\bfseries,title=STUDENT INFO,pad at break=1mm, break at=15cm/0pt ]\vspace{0.2cm}\noindent Name:  \begin{Form}\TextField[name=NAME, width=4cm,height=0.5cm,multiline=false, borderstyle=U, bordercolor={black}]{} \end{Form}   \hspace{0.2cm}Date:  \begin{Form}\TextField[name=DATE, width=1.5cm,height=0.5cm,multiline=false, borderstyle=U, bordercolor={black}]{}\end{Form} \\\end{tcolorbox}\end{multicols}\hfill\vspace{0.2cm}\begin{center}{\large \bfseries Results\\ EXPERIMENT {\thesection}\par\Huge
\import{\chapterlabel/}{title}
\\[5pt] \par}\vspace{0.2cm}\end{center}\par\noindent
%%%%%%%%%%%%HEADING
 \import{\chapterlabel/}{results} 
 %\clearpage\mbox{}\clearpage


\newpage
\vspace{2cm}
\begin{center}
\stepcounter{graphcounter}   
\begin{tikzpicture}
\begin{scope}[shift={(-15cm,-35cm)},rotate=0]
\draw[step=1mm,help lines] (0,0) grid (190mm,190mm);
\draw[step=10mm] (0,0) grid (190mm,190mm);
\end{scope}
\begin{scope}[shift={(-5cm,-36cm)},rotate=0]
\node[inner sep=0pt,rotate=0]   at (0,0) {Graph {\thegraphcounter} Temperature (Y axis) vs. time (X axis). Replicate 1 };
\end{scope}
\end{tikzpicture}
\end{center}


\newpage
 \vspace{2cm}
\begin{center}
\stepcounter{graphcounter}   
 \begin{tikzpicture}
 \begin{scope}[shift={(-15cm,-35cm)},rotate=0]
 \draw[step=1mm,help lines] (0,0) grid (190mm,190mm);
 \draw[step=10mm] (0,0) grid (190mm,190mm);
  \end{scope}
 \begin{scope}[shift={(-5cm,-36cm)},rotate=0]
 \node[inner sep=0pt,rotate=0]   at (0,0) {Graph {\thegraphcounter} Temperature (Y axis) vs. time (X axis). Replicate 2 };
 \end{scope}
\end{tikzpicture}
\end{center}



\newpage
 \vspace{2cm}
\begin{center}
\stepcounter{graphcounter}   
 \begin{tikzpicture}
 \begin{scope}[shift={(-15cm,-35cm)},rotate=0]
 \draw[step=1mm,help lines] (0,0) grid (190mm,190mm);
 \draw[step=10mm] (0,0) grid (190mm,190mm);
  \end{scope}
 \begin{scope}[shift={(-5cm,-36cm)},rotate=0]
 \node[inner sep=0pt,rotate=0]   at (0,0) {Graph {\thegraphcounter} Temperature (Y axis) vs. time (X axis). Low concentration. };
 \end{scope}
\end{tikzpicture}
\end{center}


\newpage
 \vspace{2cm}
\begin{center}
\stepcounter{graphcounter}   
 \begin{tikzpicture}
 \begin{scope}[shift={(-15cm,-35cm)},rotate=0]
 \draw[step=1mm,help lines] (0,0) grid (190mm,190mm);
 \draw[step=10mm] (0,0) grid (190mm,190mm);
  \end{scope}
 \begin{scope}[shift={(-5cm,-36cm)},rotate=0]
 \node[inner sep=0pt,rotate=0]   at (0,0) {Graph {\thegraphcounter} Temperature (Y axis) vs. time (X axis). High concentration. };
 \end{scope}
\end{tikzpicture}
\end{center}


%%%%%%%%%%%%HEADING
\newgeometry{right=5cm}\thispagestyle{sidebar}\sidebar{\vspace{1em}\vspace{1cm}\vfill\par\vspace*{1em}}\restoregeometry\clearpage \begin{multicols}{2}\begin{tcolorbox}[enhanced jigsaw,breakable,size=title,colback=mybrown!05,colframe=black,fonttitle=\bfseries,title=STUDENT INFO,pad at break=1mm, break at=15cm/0pt ]\vspace{0.2cm}\noindent Name:  \begin{Form}\TextField[name=NAME, width=4cm,height=0.5cm,multiline=false, borderstyle=U, bordercolor={black}]{} \end{Form}   \hspace{0.2cm}Date:  \begin{Form}\TextField[name=DATE, width=1.5cm,height=0.5cm,multiline=false, borderstyle=U, bordercolor={black}]{}\end{Form} \\\end{tcolorbox}\end{multicols}\hfill\vspace{0.2cm}\begin{center}{\large \bfseries Post-lab Questions \par\Huge
 \import{\chapterlabel/}{title}
\\[5pt] \par}\vspace{0.2cm}\end{center}\par\noindent\uline{  \hfill \normalsize \hfill       }
%%%%%%%%%%%%HEADING
 \import{\chapterlabel/}{postlab}
 \clearpage\mbox{}\clearpage


 \end{refsection}
\end{document}
