...
%\label{ch:atoms}
      ...
      \begin{marginfigure}
      \begin{tikzpicture} \node (a) at (0,0) {\includegraphics[width=4cm]{../Ch-kinetics/figure1}} node[rotate=90, font=\tiny] at ([yshift=.5cm,xshift=.1cm]a.south east) {\textsuperscript{\textcopyright} PxFuel} ;
\end{tikzpicture}
\end{marginfigure}
...
\begin{minipage}[b]{1.0\linewidth}\resizebox{1.0\textwidth}{!}{
\begin{tikzpicture}
 \begin{scope}[shift={(0,0)}] %transform canvas={scale=0.15}, 
       \node[rotate=45] (a) at (0,1em) {\includegraphics[width=0.5cm]{../Ch-kinetics/delivery}}     node[rotate=0] (a) at (5.5em,1em) {\includegraphics[width=0.25cm]{../Ch-kinetics/home}}   ;
  \draw[clip]  decorate [decoration={random steps,segment length=3pt,amplitude=1pt}]  {(0,0) -- (0.5,0.6) -- (1,2) -- (1.5,0.6) -- (2,0)} -- (2,-0.7) -- (0,-0.7) -- (0,0);
    \fill[brown](0,-.7) rectangle (2,2);
    \fill[white]   decorate [decoration={random steps,segment length=3pt,amplitude=1pt}]
    {(.75,2) -- (0.85,1.5) -- (0.92,1.2) -- (0.99,1.7) -- (1.05,1.4) -- (1.3,2)} -- (0,2);
     \end{scope}
 \begin{scope}[shift={(10em,0em)}] %transform canvas={scale=0.15}, 
       \node[rotate=45] (a) at (0,1em) {\includegraphics[width=0.5cm]{../Ch-kinetics/delivery}}     node[rotate=0] (a) at (5.5em,1em) {\includegraphics[width=0.25cm]{../Ch-kinetics/home}}   ;
  \draw[clip]  decorate [decoration={random steps,segment length=3pt,amplitude=1pt}]  {(0,0) -- (0.5,0.6) -- (1,1.5) -- (1.5,0.6) -- (2,0)} -- (2,-0.7) -- (0,-0.7) -- (0,0);
    \fill[green](0,-.7) rectangle (2,2);
    \fill[white]   decorate [decoration={random steps,segment length=3pt,amplitude=1pt}]
    {(.75,1.8) -- (0.80,1.3) -- (0.92,1.0) -- (0.99,1.5) -- (1.05,1.2) -- (1.3,1.8)} -- (0,1.8);
     \end{scope}
 %    \node[text width=5cm, fontscale=0.1, shift={(9,-0)}] at (-8em,-5em) { \begin{bf}\color{black}\bfseries\large Figure \ref{Fig:{\chapterlabel}\thefigurenewcounter} \end{bf} A fast and a slow reaction pathway  };
\end{tikzpicture}
}\end{minipage}
...
\hspace{2cm}\vspace{1cm}  \import{files/}{Figure-Collision-theory} \newpage

\item[\docfilehook{Catalysts}{}] 
...
{};
  ...
  \draw[->,thick,blue,shorten >=2mm,shorten <=2mm] (-3.8,-2.5) -- (10,-2.5) node [midway, below] {Reaction coordinate};
  \draw[->,thick,blue,shorten >=2mm,shorten <=2mm] (-3.5,-2.8) -- (-3.5,7) node [midway, below, rotate=90] {Energy};
  \draw[<->,thick,red,shorten >=2mm,shorten <=2mm, shift={(-3.5em,0.0em)}] (3.5,-0.3) -- (3.5,3.5) node [fill=white,rounded corners=2pt,inner sep=1pt,midway, below, rotate=0, shift={(0,0)}] {Activation energy};
 \node[text width=12cm, fontscale=.2,shift={(4em,-10em)}] at (0em,0em) { \begin{bf}\color{black}\bfseries\large Figure \ref{Fig:{\chapterlabel}\thefigurenewcounter} \end{bf} Energy diagram showing the catalyzed and non-catalyzed pathways  };
\end{tikzpicture}
}\end{minipage}
\item[\docfilehook{Heterogeneous catalysis}{}] 
...
\begin{minipage}[b]{1.0\linewidth}\resizebox{1.2\textwidth}{!}{
    ...
    \foreach \x in {1.25,1.75,2.25,2.75,3.25,3.75,4.25,4.75}
            \draw [electron] (\x,1.25,-0.5) circle (0.1cm);
    \foreach \x in {1,1.5,2,2.5,3,3.5,4,4.5,5,5.5,6,6.5,7,7.5}
            \draw [electron] (\x,0.75,-0.5) circle (0.1cm);

    %Draw the O2 molecules
    \draw [oxygen] (1.5,2.0,-0.5) circle (0.15cm);  \draw [oxygen] (1.6,2.2,-0.5) circle (0.15cm);
    \draw [nitrogen] (1.8,1.8,-0.5) circle (0.15cm);   \draw [nitrogen] (2.05,1.8,-0.5) circle (0.15cm);
    \draw[->] (2.5,2.5)--(3,2.5) node[anchor=south, shift={(-5em,0em)}]{Gas} node[anchor=south, shift={(0em,-1em)}]{Adsorption};
        \draw [oxygen] (3.5,2.0,-0.5) circle (0.15cm);  \draw [oxygen] (3.6,2.2,-0.5) circle (0.15cm);
    \draw [nitrogen] (3.0,1.7,-0.5) circle (0.15cm);   \draw [nitrogen] (3.05,1.4,-0.5) circle (0.15cm);
    \draw[->] (4.0,1.7)--(4.5,1.7) node[anchor=south, shift={(-1em,0em)}]{Reaction}  ;

   \draw [nitrogen] (4.49,1.4,-0.5) circle (0.15cm);   \draw [nitrogen] (5.19,1.3,-0.5) circle (0.15cm);
      \draw [oxygen] (4.5,1.7,-0.5) circle (0.15cm);  \draw [oxygen] (4.6,2.0,-0.5) circle (0.15cm);
       \draw[->] (6.0,1.3)--(6.5,1.3) node[anchor=south, shift={(-1em,0em)}]{Desorption}  ;

    \draw [nitrogen] (7.49,1.4,-0.5) circle (0.15cm);   
      \draw [oxygen] (7.5,1.7,-0.5) circle (0.15cm);  \draw [oxygen] (7.6,2.0,-0.5) circle (0.15cm);
      
    \draw [nitrogen] (6.7,0.9,-0.5) circle (0.15cm); 


       \node[text width=10cm,shift={(9,2)}] at (-8em,-5em) { {\fontsize{5}{6}\selectfont  {\fontsize{6}{6}\selectfont \begin{bf}\color{black}\bfseries Figure \ref{Fig:{\chapterlabel}\thefigurenewcounter} \end{bf} }Steps in a heterogeneously catalyzed reaction}  };
