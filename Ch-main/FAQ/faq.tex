...
\usepackage{movie15}\usepackage{xpatch}%
\usepackage{hyperref}
...
%\author[D. Torres, \today]{D. Torres} % Author
...
% \usepackage[style=chem-acs, backend=bibtex]{biblatex}\usepackage[english]{babel}
...
\usepackage[most]{tcolorbox}\newtcolorbox{answerbox}{colback=white,colframe=white,frame hidden, enhanced, breakable}
...
\vfill % Whitespace between the title and the author name
 ...
 \vfill 
{\Large\textbf{D. Torres \& Luis Gonzalez-Urbina}}\\ % Author name
...
%%%%%%%OLD TITLE IMAGE HEADING%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\DeclareFixedFont{\chapternumberfont}{T1}{ppl}{}{}{1.5in}
%\titleformat{name=\subsection, numberless}[display]{\Huge\bfseries\sffamily\color{black}\printcurrentpicture     }{}{-1.75in}{}[\vspace*{1in}]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%TITLE IMAGE HEADING%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \titleformat{\chapter  }[display]{\Huge\sffamily\color{black}\printcurrentpicture \vspace*{3.5cm} }{}{-1.75in}{Ch.  \thechapter.     }[\vspace*{0.5in}]\titlespacing{\chapter}{0pt}{0pt}{30pt}\newcommand{\printcurrentpicture}{ \vbox{ \hbox{ \makebox[0pt][l]{ \hspace*{-3cm}\raisebox{0cm}[0pt][0pt]{ \resizebox{\paperwidth}{!}{\begin{tikzpicture}\coordinate (a) at (0,0);\path[fill overzoom image={\thechapterpicture}]   (0,0) rectangle (\textwidth,4cm)  node[rotate=0, font=\tiny] at ([yshift=.0cm,xshift=15cm]a.south east) {\textsuperscript{\textcopyright} \thechapterpicturelabel} node[rotate=-90, font=\tiny] at ([yshift=1.5cm,xshift=15cm]a.south east) {  \today \vspace{0.1cm}  (\currenttime)}; \end{tikzpicture}}}} }}}
%\newcommand{\chapterpicture}[1]{\def\thechapterpicture{#1}}
%\newcommand{\chapterpicturelabel}[1]{\def\thechapterpicturelabel{#1}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\tikzset{
  tube/.pic={
    ...
    \begin{scope}[black]
      ...
      \foreach \y/\x in {-5/1,-4/1.5,-3/2,-2/2.5}{
...
\tikzset{
 ...
 \setchemfig{double bond sep=0.35700 em, atom sep=1.78500 em,bond offset=0.18265 em,line width = \bondwidth }\newcommand{\bondwidth}{0.06642 em}
 \usepackage[strict]{changepage}\usepackage{ulem}
  \usepackage{ physics }
...
\newtcolorbox{cornerbox}[1][]{%
  enhanced jigsaw,
  boxrule=0pt,
  northwestsoutheast,breakable=true,title=Unedited text,
  #1
}
...
\DefineLCDchar{rz}{1110010100101001110000000000000000000000}
\DefineLCDchar{riseten}{1011110101101011011100000000000000000000}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 %%LAYOUTPROBLEMSWORKSEETPAGE%%%%%%%%%%%% 
\newcommand\ProblemSectionlabel{}
...
\renewcommand\LayoutCheckField[2]{%
 ...
 %%LAYOUTPROBLEMSWORKSEETPAGE%%%%%%%%%%%% 
   ...
   %%%%%%% FILLABLE NAME AND DATE FOR QUIZZES
\newcommand{\FillableNamebox}{\begin{Form}\renewcommand{\arraystretch}{1.5} \noindent
         \begin{tabular}{@{}A{ 0.35\textwidth }A{ 0.1\textwidth }@{}}
              &  \\ 
            \DefaultTextField[name=Firstname]{Full Name:} & \today\\ 
\end{tabular}\end{Form}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%RANDOM NUMBER
  \usepackage[]{lcg}
...
%\fancyfoot[L,R]{%
%     \hspace{0.5\textwidth}%
%    \raisebox{1.5ex}{\large\normalfont \thepage}%
%}

 \fancyfoot[L]{ \thepage}\fancyfoot[C]{}\fancyfoot[R]{ \thepage}
...
% \pagestyle{fancy}
%\newcommand{\Xfruit}{\phantom{X}} % Filler to define baseline of empty circles
%\fancyhf{}
...
%\providetoggle{chem121}\settoggle{chem121}{\chemversionhealthalied}\iftoggle{chem121}{\title{GOB Chemistry Experiments}       }{ \title{College Chemistry}}
 

 \usepackage{array}\usepackage{wasysym}
...
\newlistof{questionsA}{faq}{\large List of Frequently Asked Questions} % This creates a new table of contents-like environment that will output a file with extension .faq
...
\newcommand{\questionA}[1] % This is what you will use to create a new question
{
\refstepcounter{questionsA} % Increases the questions counter, this can be referenced anywhere with \thequestions
\par\noindent % Creates a new unindented paragraph
\phantomsection % Needed for hyperref compatibility with the \addcontensline command
\addcontentsline{faq}{questionsA}{#1} % Adds the question to the list of questions
...
%
\begin{center}
\Huge{\bf \emph{Chemistry Class FAQ's}} % Main title
\end{center}

\listofquestionsA % This prints the subtitle and a list of all of your questions
...
\questionA{When should I come to class in a hybrid class?}\label{new-question}

Hybrid classes in chemistry meet during the lab session. If you access CUNY first you will find the class meeting days and times next to the class information. For example, it would say Chem 202 Wednesday 5-7.45 PM. That means the class meets weekly, assuming BMCC is open, on Wednesdays at 5 pm sharp. You also need to consult BMCC's calendar at https://www.bmcc.cuny.edu/academics/academic-calendar/ to find out what dates the school is open. Your instructor will never tell you if there is a class next week, or even the present week as he or she, like you, needs to consult CUNY first and BMCC's calendars. As instructors may have several sections in order not to give misleading information, it is recommended to suggest students check the calendars.

 
\questionA{Do we have videos in a hybrid chem class?}\label{labels}

The answer is no, there are no prerecorded videos in your chem class made by the instructor. During the meeting sessions, your instructor will have time to lecture on selected subjects, and those lectures are neither recorded nor can be recorded due to privacy issues. If you want a class in which the instructor lectures through videos you may wish to contact the instructor in hand to make sure that is the preferred method of teaching.
Still, our textbook comes with free youtube videos: \url{https://www.youtube.com/playlist?list=PLykBd3LSjfsTaaqeHEqiq6Blz4Oar7P2P}

 
\questionA{What if I miss a lab session?}\label{change-title}

If you snooze you lose. There are no make-ups for a missed lab. Your grade will be affected by that.

\questionA{When is the lecture on a hybrid chem lab?}\label{change-title}

There are none. You will not sit down in a classroom and listen to the instructor's lecture about chemistry. Still, at the beginning of the lab session, your instructor will cover chemistry material. 

\questionA{I don't like the textbook! What can I do?}\label{change-title}
Many textbooks cover the same chemistry content. Check the syllabus for additional bibliography and find a textbook you feel comfortable with. Your learning should not be tied up to a specific source, learn as much as you can from as many sources as you can.
Just in case, here are some other books you might want to use: (a) Zumdahl, Zumdahl,  DeCoste, Chemistry from Cengage  (b) Chang, Goldsby Chemistry from McGraw Hill (c) Rosenberg, Epstein, Krieger, Schaum's Outline of College Chemistry from Schaum (it has many solved problems). You can find them all for very little money on Amazon.
 

\questionA{It's my first time in CHEM lab. What should I do?}\label{change-title}
Make sure you print the lab experiment due for that day, and that you work on the prelab before entering the lab. The lab manual can be found in the LMS. An experiment is covered in every class; the list of experiment names was given to you on the first day of class (if you missed the first day of class, read FAQ questions regarding the subject) and can be found in the chemistry lab. Don't miss your goggles, and dress properly: no short sleeves, short pants, and tie your hair if it's long. Go over the experiment and make sure you understand what to do.
\questionA{I missed the first lab session. What should I do?}\label{change-title}
The first lab session introduces the chemistry lab. However, if you snooze you lose. There are no make-ups for a missed lab. Ask your coworkers about what you missed. Do not contact your instructor: it is your responsibility to come to class, and you should not cause extra work for your unreadiness. If you signed up late to class, it is your bad, you'll get the leftovers. Next time make sure you respect the deadlines and be ready when class enrolment starts not months later.


\questionA{My BMCC ID number does not work when submitting the HW. What should I do?}\label{change-title}
If you signed up to class late, switched classes or were part of the waiting list, your BMCC id will not work for the HW for the first weeks of class. While you are being placed into the system, you should use 12345 as your ID when submitting HW.

\questionA{Do we have ppts in this class?}\label{change-title}
Our textbook comes with a set of free ppt: 
\begin{center}
\url{https://oercommons.org/authoring/22997-openstax-chemistry-ch-1-17-enhanced-lecture-slides/view} \end{center}However, I do not use ppt for my lectures. I lecture on the board. So get ready to take notes.


\questionA{I do not see the attatch button when submitting an assignment}\label{change-title}
Sometimes you will see that the attach button is not present when submitting an assignment or quiz. One way to address this issue is to submit a blank assignment and then try resubmitting. Very often, when resubmitting, you will be able to see the attach button. This is because BS gets stuck sometimes. If this doesn?t work, then you need to contact your instructor.


\questionA{What can I bring to a quiz?}\label{change-title}
You may bring a cheat sheet on 8.5x11 paper to the exam, written on both sides. No more than a single sheet will be allowed under any circumstance. In that sheet, which should contain a periodic table, you can write anything you want. {\bf If you decide to use a cheat sheet, you will have to hand it with your quiz and it will not be returned}. You can also bring a calculator.


\questionA{How do I access old chapters in the LMS}\label{change-title}
The LMS will only display the current chapter being covered in class. You will find this chapter in the weekly guide by means of an url. Still you can access old chapters by changing the URL and, for example, typing Ch2.pdf instead of the current Ch3.pdf.

\questionA{How will the quizes be?}\label{change-quiz}
The quizzes will be a randomized series of literal questions from the end of the chapters delivered through the LMS. It is to your advantage to work on as many end-of-the-chapter problems as you want based on your target grade.

\questionA{I cannot attend the final. What will happen?}\label{nofinal}
The final quiz is the most important quiz of the class. You take this class just to be able to take the final quiz. You cannot miss that date under any circumstances, unless you want to fail the class. If you do not take the class, your average will be computed without that grade (with zero as the grade for the final). 




\questionA{How to know when there is a class?}\label{whenclass}
To determine when a class is scheduled, please check the Course Guide in the LMS. It's in the Course Information Menu. There, you will find a list of all the dates you are required to attend class. You can also check the BMCC calendar. Both sources should give you the same information.

\questionA{What lab are we doing today?}\label{whatlab}
The lab carried every week changes based on your section. The calendar with the list of labs and dates is posted in the chemistry lab and was also given during the first day of class for those students who attended. You could stop by the chem lab anytime you are in the school. Also, you would be able to check the calendar and print the lab at the beginning of the lab session.

\questionA{Is there any specific format for the HW submission?}\label{HWformat}
Unless the HW is submitted with a specific format, the grading system will automatically grade it as zero. That could have been your case. These are the rules: (a) The HW needs to be answered in the pdf form provided, not on a different page. (b) Your name needs to be written by you, even if it’s written in the background. (c)The final answers should be written in the first two pages. (d) Your work needs to be presented in the last two pages. (e) It's permitted for you to print and handwrite the answers, but only if you scan back the pages and submit respecting the form format. (f) You can use a pdf edittor software to handwrite the pdf file on your computer if you respect the rules above.


\questionA{What am I supposed to submit for the lab work?}\label{labwork}
Lab reports are due every Sunday. You need to submit the instructor-signed prelab, the instructor-signed results, and the postlab with all questions answered.
