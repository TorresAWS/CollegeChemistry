...
\usepackage{paracol}
\usepackage{chemfig}\input{/usr/local/texlive/2025/texmf-dist/tex/generic/chemfig/chemfig-lewis.tex}
...
%%%%%%BOXES SAMPLE PROBLEM
\newcommand\textlcsc[1]{\textsc{#1}}   %FORCE CAPS
%%%%%%COLOED CIRCLES
...
\DeclareInstance{tasks}{myitemize}{default} { }\settasks{style = myitemize, column-sep = 2em,counter-format=\alph*)  , item-indent = 1.2em, label-width =1.2em, label-offset = 0.5em, after-item-skip = 1ex, after-item-skip=-1pt}
\newcommand\score[2]{%
...
\newcommand{\mytitle}[3]%
    ...
    \begin{tikzpicture}[font=\sffamily]
        ...
        \def\colb{cyan!80!white}
        \def\colc{yellow!80!white}
        \def\cold{red!80!white}
        \begin{scope}[shift={(0,-1)}]
        \node[fill=\colb,signal,signal to=east,text=black,minimum height=8mm] (arg1) at (0,0) {Face2face};
        \node[right=2mm of arg1,text=black] (arg2) {#1};
        \end{scope}
         \begin{scope}[shift={(-0.3,-2)}]
        \node[fill=\colc,signal,signal to=east,text=black,minimum height=8mm] (arg1) at (0,0) {HW};
        \node[right=7.5mm of arg1,text=black] (arg2) {#2};
        \end{scope}
        \begin{scope}[shift={(-0.3,-3)}]
        \node[fill=\cold,signal,signal to=east,text=black,minimum height=8mm] (arg1) at (0,0) {Quiz};
        \node[right=8mm of arg1,text=black] (arg2) {#3};
        \end{scope}
...
\newcommand{\makeannouncement}[4]
{
\thispagestyle{empty}
\begin{fullwidth}
\begin{myex} 
\begin{flushright}\begin{minipage}{0.9\linewidth}\subfile{#4}\end{minipage}\end{flushright}
\end{myex}
\begin{learn}
\mytitle{\begin{inparaenum}[(a)]  \subfile{#1} \end{inparaenum}}{\begin{inparaenum}[(a)]  \subfile{#2} \end{inparaenum}}{\begin{inparaenum}[(a)]  \subfile{#3} \end{inparaenum}}
\end{learn}
\end{fullwidth}
} 

%%%%%%%%%%%% ANNOUNCEMENTTS
%%%%%\makeannouncement{f2f.tex}{homew.tex}{q.tex}{abstract.tex}

%%%%%% Conductometer speedometer diagram
...
\newcommand\chapterpathtocontent{}\renewcommand\chapterpathtocontent{../}
\newcommand\chapterpathtoimages{}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% this part if for printing only odd/even answers 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\ExplSyntaxOn
...
%        \int_if_even_p:n  { \l_exsheets_counter_qu_int } &&
...
\newcommand\ChapterNameLabel{Ch-biochemistry}
%\newcommand\ChapterNameLabel{Ch-orgo}                   %%Chapter name eg Ch-orgo
\newcommand{\ProblemName}{Problems.tex}
\newcommand\HideProblems{false}                                         %%TRUE for hiding end of chapter problems
\newcommand\HideContent{false}                                  %%TRUE for hiding end of chapter problems
\newcommand\chemversionhealthalied{false}                       %%TRUE for chem 121 version FALSE for 201-2
\newcommand\showfinalanswers{true}                              %%TRUEfor showing answers false for not
\newcommand\ShowUneditedText{true}                              %%TRUEfor showing unedited text
\newcommand\printOddAnswers{true}                              %%TRUE for printing only odd answ, truefor all 
\providetoggle{chem121}\settoggle{chem121}{\chemversionhealthalied}\iftoggle{chem121}{\title{GOB Chemistry}       }{ \title{College Chemistry}}
\begin{document} 
\providetoggle{hideContentEnd}\settoggle{hideContentEnd}{\HideContent}\iftoggle{hideContentEnd}{       }{ \subfile{../\ChapterNameLabel/\ChapterNameLabel.tex}  }
\providetoggle{hideproblemsEnd}\settoggle{hideproblemsEnd}{\HideProblems}\iftoggle{hideproblemsEnd}{       }{ \subfile{../\ChapterNameLabel/\ChapterNameLabel\ProblemName}  }
\end{document} 
